...
With this formula, it is easier to see why two independent random variables have covariance of 0, since for two independent random variables X and Y, $E[XY] = E[X]E[Y]$. However, the case for the converse is not true. 
...
Clearly the relationship between the two random variables is not linear, thus covariance would not be a good method to measure the strength of relationship between the two random variables.
...
Most of the times population parameters will be unknown and I'll have to use samples to calculate the covariance between random variabls. The formula for sample covariance is as follows:

$$Cov[X,Y] = \dfrac{1}{n-1}\sum_{i=i}^{n}(X_i - \bar{X})(Y_i - \bar{Y})$$

Geometrically speaking, covariance could be seen as the sum of area of rectangles created.